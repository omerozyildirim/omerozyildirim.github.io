...
var i, x, y, w, h, dx, dy, dw, dh, x0, y0, xmin, ymin, xmax, ymax, distname, gx, gy, gz;

function set_svg_size(){
	document.getElementById("kucukcekmece").setAttribute("width", document.getElementById("svg_layer").clientWidth);
	document.getElementById("kucukcekmece").setAttribute("height", document.getElementById("svg_layer").clientHeight);
}
function init(){
	set_svg_size();
...
function clear_colors(){
	for (i = 0; i < dists.length; i++){
		document.getElementById("kucukcekmece").getElementById(dists[i][0]).setAttribute("fill", dists[i][2]);
	}
}

function svg2gmap(dummy1, dummy2){ // x, y
	var long =  7.61767643803e-06 * dummy1 + 28.7402631271;
	var lat  = -5.77075711339e-06 * dummy2 + 41.0619208673;
	return [long, lat];
}

function gmap2svg(dummy1, dummy2){ // long, lat
	var x =  131263.903403 * dummy1 - 3772558.66948;
	var y  = -173285.018631 * dummy2 + 7115415.83213;
	return [x, y];
}
