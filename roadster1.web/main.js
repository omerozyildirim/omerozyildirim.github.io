...
function calc_coefs(){
  ...
  for(i = 1; i < points.length - 2; ++i){
    dummy1 = calc_coef((i-1), points[i-1].x, i, points[i].x, (i+1), points[i+1].x, (i+2), points[i+2].x);
    dummy2 = calc_coef((i-1), points[i-1].y, i, points[i].y, (i+1), points[i+1].y, (i+2), points[i+2].y);
...
function plot_points(){
  var i;
  for (i = 0; i < points.length; i++) plot_on_canvas(points[i].x, points[i].y);
}
function draw_curve(){
  calc_coefs();
  var canvas = document.getElementById("myCanvas");
